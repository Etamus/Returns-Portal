...
export function initNovaSolicitacao(state, callbacks) {
    ...
    ui.consultarBtn.addEventListener('click', async () => {
        appState.carrinho = []; appCallbacks.updateCartView();
...
function attachResultsAreaListeners(data) {
    ui.resultsArea.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.item-summary') && !addItemBtn) { const row = e.target.closest('.item-row'); if (!row.classList.contains('added-to-cart')) { document.getElementById(`details-form-${e.target.closest('.item-summary').dataset.index}`).classList.toggle('open'); } }
        ...
        if (addItemBtn) {
            const index = addItemBtn.dataset.index; const form = document.getElementById(`details-form-${index}`);
            const fileInput = form.querySelector('.file-input'); let anexo_data = null; let anexo_filename = null;
            ...
            appState.carrinho.push({ index: index, nota_fiscal: ui.nfInput.value, cod_cliente: appState.dadosOriginaisNF.cod_cliente, nome_cliente: appState.dadosOriginaisNF.nome_cliente, material: itemData.material, cod_item: itemData.cod_item, descricao: itemData.descricao, motivo: form.querySelector(`#motivo-${index}`).value, quantidade: form.querySelector(`#quantidade-${index}`).value, observacao: form.querySelector(`#observacao-${index}`).value, anexo_filename: anexo_filename, anexo_data: anexo_data });
